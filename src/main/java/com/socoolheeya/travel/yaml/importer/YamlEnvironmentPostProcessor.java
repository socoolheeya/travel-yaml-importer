package com.socoolheeya.travel.yaml.importer;
...
public class YamlEnvironmentPostProcessor implements EnvironmentPostProcessor, Ordered {
    ...
    public void postProcessEnvironment(ConfigurableEnvironment environment, SpringApplication application) {
        log.info("** START YamlEnvironmentPostProcessor **");
        ...
        for(Resource resource : resources) {
            this.loadYaml(resource, environment)
            ...
            List<PropertySource<?>> propertySourceList = this.loader.load(path.getFilename(), path).stream().filter(
                    propertySource -> {
                        ...
                        String[] profiles = Stream.concat(
                                        ...
                                        Arrays.stream(loadProfiles(binder, SPRING_CONFIG_ACTIVATE_ON_PROFILE))
                                )
                                .toArray(String[]::new);
    ...
    private String[] loadProfiles(Binder binder, String profileConfig) {
        String[] profiles = binder.bind(profileConfig, Bindable.of(String[].class))
                .orElse(new String[0]);
        if(SPRING_PROFILES.equals(profileConfig) && profiles.length > 0) {
            log.warn("'{}' is deprecated since spring boot 2.4 and later.", SPRING_PROFILES);
        }
        return profiles;
    }
    ...
    private boolean matchProfiles(String profile, PropertySource<?> propertySource) {
        return matchProfiles(profile, propertySource, SPRING_PROFILES)
                || matchProfiles(profile, propertySource, SPRING_CONFIG_ACTIVATE_ON_PROFILE);
    }
